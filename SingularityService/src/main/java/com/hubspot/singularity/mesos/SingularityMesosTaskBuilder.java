...
class SingularityMesosTaskBuilder {
  ...
  private void prepareEnvironment(final SingularityTaskRequest task, SingularityTaskId taskId, CommandInfo.Builder commandBuilder, final int[] ports) {
    ...
    for (int portNum = 0; portNum < ports.length; portNum++) {
      if (portNum == 0) {
          setEnv(envBldr, "PORT", ports[portNum]);
      }

      setEnv(envBldr, String.format("PORT%s", portNum), ports[portNum]);
    }
